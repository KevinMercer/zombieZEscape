print("导入data_custom.lua")
-- 游戏初始化数值设定
gameInitializeData = {
    -- 在这里设定你想要一回合游戏持续多久，单位是秒(例如填480，即表示每一回合持续时间480秒，也就是8分钟，不包括开局选母体的时间)
    roundTime = 480,
    -- 在这里设定你想要多久选出母体，单位是秒
    -- (注意，这里填的时间要和地图里对应的时间装置方块里填的时间数值相等或者稍比地图里时间装置方块里甜的时间大1秒)
    readyTime = 20,
    ...
    -- 是否允许玩家复活 true为允许，false为不允许，此处改动会导致玩家可以在游戏中途加入游戏并且在回合还没有重新开始的时候就直接复活
    -- 没有lua基础的请不要轻易修改
    respawnable = false,
    -- 复活的等待时间 整数，默认60秒，如果respawnable为false的话则无效
    respawnTime = 0,
    -- 是否允许伤害敌对阵营 true为允许false为不允许，不允许的话CT和T不能击剑还玩什么逃杀？
    enemyfire = true,
    -- 是否允许队友伤害 true为允许false为不允许，你自己看着办
    friendlyfire = false,
    -- 是否允许破坏地图 true为允许false为不允许，你自己看着办
    breakable = false
}
...
zombieTable = {
    {
        ...
        maxarmor = 6000,
        flinch = 0.8,
        knockback = 0.8, -- 当前版本没用
        maxspeed = 1.15, -- 当前版本没用
        gravity = 0.695,
        jumpRate = 1.29,
        speedRate = 1.06,
        jumpLevel = 1,
